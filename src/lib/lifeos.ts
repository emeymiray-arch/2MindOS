...
/** @deprecated Use goalsInDerivedBucket from plan-calendar. */
export function goalsInBucket(
  store: LifeStore,
  _bucket: "foundation" | "development" | "later"
) {
  return (store.goals ?? []).filter((g) => !g.archived && g.active);
}
...
export function recomputeFromTaskToggle(store: LifeStore, task: DailyTaskItem): void {
  ...
  if (task.milestoneId && task.workPlanId) {
    ...
    if (ms && plan && phase) {
      // Same-day incomplete siblings shouldn't block; historical days only matter if still open today.
      const today = task.date;
      const relatedToday = (store.dayTasks ?? []).filter(
        (t) => t.milestoneId === ms.id && !t.archived && t.date === today
      );
      if (relatedToday.length) {
        ms.done = relatedToday.every((t) => t.done);
      } else {
        ms.done = Boolean(task.done);
      }
      // If user completed this task, force module done (sequential unlock).
      if (task.done) ms.done = true;
