...
export interface Goal {
  ...
  bucket?: PlanBucket;
  /**
   * Slot on the life horizon (2-month этап):
...
export interface WishItem {
  ...
  archived?: boolean;
  /** When marked purchased / acquired */
  doneAt?: string;
  /** Target price for material wishes (₽). */
  targetAmount?: number;
  /** Sum of savings txs tagged to this item. */
  savedToward?: number;
}
...
export interface FinanceSummary {
  ...
  /** Monthly salary baseline (ЗП), set separately from transactions. */
  salary: number;
  cushion: number;
  /** If true, cushion is manual override; otherwise derived from savings txs. */
...
export interface DailyTaskItem {
  ...
  projectId?: string;
  habitId?: string;
  /** Stable key for auto-composed tasks (plan, habit, project, week). */
  autoSource?: string;
  priority?: TaskPriority;
  deadlineStart?: string;
  deadlineEnd?: string;
  /** Denormalized sum of logged minutes for this task */
...
export type QuestKind = "daily" | "weekly" | "milestone" | "phase";
