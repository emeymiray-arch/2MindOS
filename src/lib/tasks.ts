...
export function ensureDayTask(
  ...
  if (patch.autoSource) {
    const existing = store.dayTasks.find(
      (t) => t.date === patch.date && t.autoSource === patch.autoSource
    );
    if (existing) {
      const keepDone = patch.done === undefined ? existing.done : Boolean(patch.done);
  ...
  if (patch.stageId && !patch.milestoneId) {
    const existing = store.dayTasks.find(
      (t) =>
        t.date === patch.date &&
        t.stageId === patch.stageId &&
        !t.milestoneId &&
        !t.archived
    );
    if (existing) {
      const keepDone = patch.done === undefined ? existing.done : Boolean(patch.done);
      Object.assign(existing, patch, { id: existing.id, done: keepDone });
      return existing;
    }
  }
