...
import { writePortableBundle, writeImportInstructions, writeLatestPortableBundle, writeStampedPortableBundle, writeVaultReadme } from "./portable-backup";
...
declare global {
  ...
  // eslint-disable-next-line no-var
  var __mindosLastPortableExportAt: number | undefined;
  // eslint-disable-next-line no-var
  var __mindosLastStampedExportAt: number | undefined;
  // eslint-disable-next-line no-var
...
/** Finder-visible vault: ~/Documents/2MindOS (override with MINDOS_DATA_DIR). */
...
function dataDir(): string {
  return path.join(getVaultRoot(), "data");
}

function legacyProjectDataDir(): string {
...
async function copyDataTree(fromDir: string, toDir: string): Promise<void> {
...
/** One-time: move richer copy from project data/ → ~/Documents/2MindOS/data/ */
async function migrateLegacyDataToVault(): Promise<void> {
  if (isServerless()) return;
  const vault = dataDir();
  const legacy = legacyProjectDataDir();
  if (path.resolve(legacy) === path.resolve(vault)) return;

  await fs.mkdir(vault, { recursive: true });
  const vaultStore = await readStoreFile(path.join(vault, "lifeos.json"));
  const legacyStore = await readStoreFile(path.join(legacy, "lifeos.json"));

  if (legacyStore && (!vaultStore || storeWeight(legacyStore) > storeWeight(vaultStore))) {
    console.info("[mindos] migrating project data/ → vault", { from: legacy, to: vault });
    await copyDataTree(legacy, vault);
  }
}
...
async function ensureVaultReady(): Promise<void> {
  ...
  await migrateLegacyDataToVault();
  await writeVaultReadme(root, dir);
  await writeImportInstructions(path.join(dir, "import"), dir);
}
...
async function loadLocalCandidates(): Promise<LifeStore[]> {
  ...
  const legacy = legacyProjectDataDir();
  if (path.resolve(legacy) !== path.resolve(dataDir())) {
    files.push(path.join(legacy, "lifeos.json"), path.join(legacy, "lifeos.prev.json"));
  }
  const out: LifeStore[] = [];
...
async function maybeAutoPortableExport(store: LifeStore): Promise<void> {
  if (isServerless()) return;
  const now = Date.now();
  const layout = getStorageLayout();

  try {
    const lastLatest = global.__mindosLastPortableExportAt ?? 0;
    if (now - lastLatest >= BACKUP_EVERY_MS) {
      global.__mindosLastPortableExportAt = now;
      await writeLatestPortableBundle(
        store,
        { exportsDir: layout.exportsDir, latestExportDir: layout.latestExportDir },
        "auto"
      );
    }

    const lastStamped = global.__mindosLastStampedExportAt ?? 0;
    if (now - lastStamped >= DATED_EXPORT_EVERY_MS) {
      global.__mindosLastStampedExportAt = now;
      await writeStampedPortableBundle(
        store,
        { exportsDir: layout.exportsDir, latestExportDir: layout.latestExportDir },
        "auto"
      );
    ...
    await writeVaultReadme(layout.vaultRoot, layout.dataDir);
  } catch (e) {
    console.error("[mindos] portable auto-export failed:", e);
  }
}
...
async function persistLocal(store: LifeStore): Promise<boolean> {
  ...
  try {
    ...
    await maybeAutoPortableExport(store);
    return true;
  } catch (e) {
...
export function getStorageLayout() {
  ...
  return {
    ...
    latestExportDir: path.join(dir, "exports", "latest"),
    importDir: path.join(dir, "import"),
    legacyDataDir: legacyProjectDataDir(),
    envHint: process.env.MINDOS_DATA_DIR?.trim() || vaultRoot,
    finderPath: vaultRoot,
  };
}

export async function exportPortableNow(): Promise<{
  ok: boolean;
  manifest?: import("./portable-backup").PortableManifest;
  paths?: ReturnType<typeof getStorageLayout>;
  error?: string;
}> {
  if (isServerless()) {
    return { ok: false, error: "serverless — use browser export" };
  }
  try {
    const store = await ensureLoaded();
    const paths = getStorageLayout();
    const { manifest } = await writePortableBundle(
      store,
      { exportsDir: paths.exportsDir, latestExportDir: paths.latestExportDir },
      { kind: "manual" }
    );
    await writeImportInstructions(paths.importDir, paths.dataDir);
...
export async function importFromDataFolder(
  ...
  if (isServerless()) {
    return { ok: false, restored: false, error: "serverless — use POST import" };
  }
  const { loadImportCandidate, mergeImportDecision } = await import("./portable-backup");
  const paths = getStorageLayout();
  const { store: incoming, source } = await loadImportCandidate(paths.importDir);
  if (!incoming) {
    return {
      ok: false,
      restored: false,
      error: "В data/import/ нет lifeos.json или папки latest/",
    };
  }
  const current = await ensureLoaded();
  const { store: picked, strategy } = mergeImportDecision(current, incoming, mode);
  if (strategy === "kept-current") {
    return {
      ok: true,
      restored: false,
      source,
      strategy,
      weight: storeWeight(current),
    };
  }
  bumpEpoch();
  global.__mindosStore = picked;
  picked.revision = (Number(picked.revision) || 0) + 1;
  await persistLocal(picked);
  void persist(picked, true).catch(() => undefined);
  return {
    ok: true,
    restored: true,
    source,
    strategy,
    weight: storeWeight(picked),
  };
}

export async function restorePortableExport(
  exportId: string,
  ...
  const { loadExportById, mergeImportDecision } = await import("./portable-backup");
  const paths = getStorageLayout();
  const { store: incoming } = await loadExportById(paths.exportsDir, exportId);
  if (!incoming) {
    return { ok: false, restored: false, error: "export not found" };
  }
  const current = await ensureLoaded();
  const { store: picked, strategy } = mergeImportDecision(current, incoming, mode);
  if (strategy === "kept-current") {
    return { ok: true, restored: false, strategy };
  }
  bumpEpoch();
  global.__mindosStore = picked;
  picked.revision = (Number(picked.revision) || 0) + 1;
  await persistLocal(picked);
  void persist(picked, true).catch(() => undefined);
  return { ok: true, restored: true, strategy };
}
