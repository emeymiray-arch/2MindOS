...
function enrich(
  ...
  const phaseGroups = groupStagesIntoPhases(plan).map((g) => ({
    ...g,
    stages: g.stages.map((ph) => ({
      ...ph,
      modules: phaseModules(ph),
      progress: milestoneProgress(ph),
    })),
...
export async function POST(request: Request) {
  ...
  if (action === "create") {
    ...
    if (ownerType === "project") {
      return NextResponse.json(
        {
          error:
            "Планы проектов пока не ведутся на Главной. Создай план у цели.",
        },
        { status: 400 }
      );
    }
    ...
    const store = await updateStore((s) => {
      ...
      const existing = workPlanForOwner(s, "goal", ownerId);
      if (existing) return;

      const g = s.goals.find((x) => x.id === ownerId);
      if (!g) return;
      const title = String(body.title ?? "").trim() || `Plan: ${g.title}`;
      const phases = stagesToPhases(g.stages ?? []);
      ...
      rescheduleAllStages(plan, s);
      s.workPlans.push(plan);
      g.workPlanId = plan.id;
      unlockNextPlanStep(s, plan.id);
      syncWorkPlanProgress(s, plan);
    });

    const created = workPlanForOwner(store, "goal", ownerId);
  ...
  if (action === "addPhase") {
    ...
    const store = await updateStore((s) => {
      ...
      const start = planStartDate(plan, s);
      const { start: ds, end: de } = stageWindow(start, order);
      const pNum = phaseNumberForStageOrder(order);
      const phase: PlanPhase = {
        id: id(),
        title: title.includes("Фаза") || title.includes("Этап")
          ? title
          : `${phaseLabel(pNum)} · ${title}`,
        order,
        durationWeeks: STAGE_WEEKS,
        deadlineStart: body.deadlineStart ? String(body.deadlineStart) : ds,
        deadlineEnd: body.deadlineEnd ? String(body.deadlineEnd) : de,
  ...
  if (action === "toggleMilestone" || action === "toggleModule") {
    ...
    const store = await updateStore((s) => {
      ...
      syncWorkPlanProgress(s, plan);
      if (ms.done) unlockNextPlanStep(s, planId, todayKey());
    });
    return NextResponse.json(enrich(store, planId, lite));
  }
  ...
  if (action === "addTaskFromModule") {
    ...
    const store = await updateStore((s) => {
      ...
      s.dayTasks.push({
        ...
        autoSource: owner.goal
          ? `goal:${owner.goal.id}:topic:${mod.id}`
          : `project:${owner.project?.id}:topic:${mod.id}`,
        workPlanId: plan.id,
        stageId: phaseId,
        milestoneId: mod.id,
      ...
      if (owner.project) {
        // Projects stay off Today — archive immediately if somehow created
        const t = s.dayTasks[s.dayTasks.length - 1];
