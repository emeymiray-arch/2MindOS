...
export async function GET(request: Request) {
  ...
  let blocks = (store.wishBlocks ?? [])
    .filter((b) => (showArchived ? b.archived : !b.archived))
    .map((b) => ({
      ...b,
      items: (b.items ?? []).map((i) => ({
        ...i,
        savedToward: savedByItem.get(i.id) ?? i.savedToward ?? 0,
      })),
    }));
  if (q) {
...
export async function POST(request: Request) {
  ...
  if (action === "addItem") {
    ...
    const store = await updateStore((s) => {
      ...
      const item: WishItem = {
        ...
        targetAmount:
          body.targetAmount != null && Number(body.targetAmount) > 0
  ...
  if (action === "toggleItem") {
    const store = await updateStore((s) => {
      ...
      if (item) {
        const next = Boolean(body.done ?? !item.done);
        item.done = next;
        item.doneAt = next ? now() : undefined;
      }
    });
    return NextResponse.json({ blocks: store.wishBlocks });
  }
  ...
  if (action === "updateItem") {
    const store = await updateStore((s) => {
      ...
      if (body.targetAmount !== undefined) {
        ...
        item.targetAmount = Number.isFinite(n) && n > 0 ? n : undefined;
      }
    });
    return NextResponse.json({ blocks: store.wishBlocks });
  }
